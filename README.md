# Мета-Кристалл v7.3 — Веб-приложение

Веб-реализация десктопного GUI «Мета-Кристалл» на базе Next.js 16 + TypeScript с интеграцией локальных LLM (Ollama), RAG-пайплайна, векторного поиска по базе знаний движка и **семантического поиска параметров Strudel** для strudel-flow.

## 🎯 Новые возможности (v7.3)

### Strudel RAG — Семантический поиск параметров

Режим семантического поиска параметров Strudel с помощью RAG (Retrieval-Augmented Generation) на основе модели BGE-m3:

- **Естественно-языковые запросы**: Поиск параметров по описанию на русском или английском языке
- **Векторный поиск**: Косинусное сходство с предвычисленными эмбеддингами
- **React Flow интеграция**: Добавление узлов в flow одним кликом
- **Fallback режим**: Keyword-based поиск при недоступности эмбеддингов

**Примеры запросов:**
- *"ретро-звук с быстрым арпеджио"* → `crush`, `arp`, `fast`
- *"космический эмбиент"* → `reverb`, `delay`, `slow`
- *"агрессивный бас"* → `distort`, `lpf`, `sawtooth`

См. полную документацию: [python_engine/strudel_rag/README.md](./python_engine/strudel_rag/README.md)
