...
import { StrudelSuggestion } from '../SemanticStrudelSuggester';
import { useStore } from '../../lib/strudel/strudel-flow-store';
...
export function CrystalBridgePanel({
  ...
  const handleTransform = useCallback(async () => {
    ...
    try {
      ...
      // Авто-применение если уверенность высокая и включено
      if (autoApply && bridge.shouldAutoApply(bridgeResult.confidence)) {
        // Применяем топ-1 рекомендацию автоматически
  ...
  }, [crystal, onSearch, autoApply, onApplied]);

  // Применение отдельного предложения
  const handleApplySuggestion = useCallback((suggestion: StrudelSuggestion) => {
    // Определяем тип узла на основе категории
    let nodeType = 'effect';
    if (suggestion.category?.includes('Instrument') || suggestion.category?.includes('Synth')) {
      nodeType = 'oscillator';
    } else if (suggestion.category?.includes('Effect')) {
      nodeType = 'effect';
    } else if (suggestion.category?.includes('Pattern') || suggestion.category?.includes('Sequence')) {
      nodeType = 'pattern';
    }
