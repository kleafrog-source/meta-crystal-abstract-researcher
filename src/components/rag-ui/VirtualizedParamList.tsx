...
export function VirtualizedParamList(props: {
  ...
  const storeParameters = useRagStore((state) => state.activeParameters);
        ...
        <div className="flex flex-col items-center justify-center gap-3 px-6 py-16 text-center">
          ...
          <div className="space-y-1">
            <p className="text-sm font-medium">
              {isSearching
                ? "Searching across vectorized Flowmusic parameters..."
                : "No active parameters yet"}
            </p>
            {!isSearching ? (
              <p className="text-xs text-muted-foreground">
                Run a semantic query above and the top ranked parameters will appear here.
              </p>
            ) : null}
          </div>
        </div>
