...
export function ParameterControl(props: {
  ...
  const updateParameterValue = useRagStore((state) => state.updateParameterValue);
  const removeParameter = useRagStore((state) => state.removeParameter);
  const numericValue =
    ...
    >
                ...
                >
                  <div className="space-y-2 text-[11px]">
                    ...
                    <div className="opacity-80">
                      Category: {props.param.category}
                      {" | "}
                      Sub: {props.param.sub_category}
                    </div>
                    {props.param.lyria_prompt_tags.length > 0 ? (
                      <div className="opacity-80">
                        Tags: {props.param.lyria_prompt_tags.join(", ")}
                      </div>
                    ) : null}
                  </div>
                </TooltipContent>
      ...
      {props.param.ui_element === "Range" || props.param.ui_element === "Toggle" ? (
        <RangeControl
          ...
          value={numericValue}
          onChange={(value) =>
            updateParameterValue(props.param.technical_name, value)
          }
        />
      ) : null}
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-1.5">
        <CompactPresetButton
          label="Min"
          onClick={() => props.onChange(roundToStep(min, min, step))}
        />
        <CompactPresetButton
          label="Def"
          onClick={() =>
            props.onChange(
              normalizePresetValue(props.param.default, min, max, step),
            )
          }
        />
        <CompactPresetButton
          label="Max"
          onClick={() => props.onChange(roundToStep(max, min, step))}
        />
        <CompactPresetButton
          label="Rnd"
          onClick={() => props.onChange(randomSteppedValue(min, max, step))}
        />
      </div>
...
function CompactPresetButton(props: {
  label: string;
  onClick: () => void;
}) {
  return (
...
function SelectControl(props: {
  ...
  const safeValue =
    props.value && options.includes(props.value) ? props.value : (options[0] ?? "");
...
function TextControl(props: {
  ...
  return (
    <div className="space-y-1">
      ...
      />
      {props.param.min_length || props.param.max_length ? (
        <div className="text-[10px] text-muted-foreground">
          {props.param.min_length ? `min ${props.param.min_length}` : ""}
          {props.param.min_length && props.param.max_length ? " | " : ""}
          {props.param.max_length ? `max ${props.param.max_length}` : ""}
        </div>
      ) : null}
    </div>
  );
