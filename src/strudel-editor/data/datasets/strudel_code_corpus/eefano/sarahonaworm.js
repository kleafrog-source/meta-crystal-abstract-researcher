...
const randvel = register('randvel', (l,pat) => pat.velocity(rand.late(l).mul(.3).add(.7)))
...
$: note(`<a6@3 g6 a#6 f#6 a6 f#6 c6 f#6 c6 
  ...
  .layer
  (x=>x.s("gm_piccolo").randvel(23).gain("<.45 .5 .55 .5>/92")
  ,x=>x.s("gm_cello").mask("<0 1 1 1>/92").transpose(-12).gain(.8).pan(.42)
  ,x=>x.s("gm_oboe").mask("<0 0 1 0>/92").transpose(-12).randvel(31).gain(.55).pan(.53)
)
$: `<D@3 D# C D C D Am D Am F 
  ...
  .layer
  (x=>chord(x).anchor("b4").voicing().s("gm_bandoneon").mask("<0 1 1 1>/92").randvel(2).gain(.25).pan(.53)
  ,x=>chord(x).anchor("b4").voicing().s("gm_string_ensemble_1").mask("<0 0 1 0>/92").att(.1).rel(.1).randvel(5).gain(.5).pan(.42)
  ,x=>n(irand(7).seg(1).pick(["0","2","2 0","0 2","2 1","~ 2","~ 1"]).sub(7)).scale(x.toscale()).late(.02)
    .s("gm_piano:3").randvel(7).gain("<.6 .65 .7 .65>/92").lpf(800)
  ,x=>n(irand(3).seg(1).pick(["1,2 1","2,3 2","[1,2] [2,3]"])).chord(x).anchor("g4").voicing().late(.05)
    .s("gm_piano:2").randvel(13).gain("<.4 .45 .5 .45>/92").pan(.51)
  ,x=>n(irand(6).seg(1).pick(["0","2 1","[0 1] 2","2 0","[2 3] 4","[4 3] 2"]).add(14)).scale(x.toscale())
    .s("gm_piano:1").randvel(19).gain("<.6 .65 .7 .65>/92").pan(.48)
)
