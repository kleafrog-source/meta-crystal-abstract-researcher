import type { TorusAtlasCrystal, TorusAtlasMetrics } from "@/types/torus-atlas";

export function buildAtlasWhere(search: string, emeralds: boolean, ids?: string[], layoutKey?: string) {
  ...
  if (layoutKey) {
    where.metadataJson = { contains: layoutKey };
  }
  if (emeralds) {
    where.type = { in: ["EMERALD", "emerald", "РР·СѓРјСЂСѓРґ"] };
  }
  if (search) {
    where.OR = [
...
export function mapCrystalToAtlas(crystal: any): TorusAtlasCrystal {
  ...
  return {
    ...
    type: crystal.type,
    clusterLabel: atlasSnapshot?.torusClusterLabel ?? atlasSnapshot?.semanticClusterLabel ?? (typeof torusDoc?.cluster === "number" ? torusDoc.cluster : 0),
    semanticClusterLabel: atlasSnapshot?.semanticClusterLabel ?? (typeof torusDoc?.cluster === "number" ? torusDoc.cluster : 0),
    torusClusterLabel: atlasSnapshot?.torusClusterLabel ?? atlasSnapshot?.semanticClusterLabel ?? (typeof torusDoc?.cluster === "number" ? torusDoc.cluster : 0),
    formulaCluster: atlasSnapshot?.semanticClusterLabel ?? (typeof torusDoc?.cluster === "number" ? torusDoc.cluster : 0),
    torusX: point3d.x,
    torusY: point3d.y,
    torusZ: point3d.z,
    torusU,
    ...
    isEmerald: String(crystal.type ?? "").toLowerCase().includes("emerald") || String(crystal.type ?? "").includes("РР·СѓРјСЂСѓРґ"),
...
function readAtlasSnapshot(metadata: Record<string, unknown>) {
  ...
  return {
    ...
    semanticClusterLabel: Number(record.semanticClusterLabel ?? record.clusterLabel ?? 0),
    torusClusterLabel: Number(record.torusClusterLabel ?? record.clusterLabel ?? 0),
  };
}
